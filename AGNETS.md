...
- 核心板：`STM32H750VB / H743VI 核心板（M版）`
- 当前工程目标芯片：`STM32H743VIT6`
- 同系列板可能焊接 `STM32H750VB` 或 `STM32H743VI`：
  - `H743VI`：片内 Flash 2048 KB，RAM 1024 KB。
  - `H750VB`：片内 Flash 128 KB，RAM 1024 KB。
...
- 板上 5 V 输入来自 Micro USB 或预留 5 V 接口，经 `AMS1117-3.3V` 生成 `3V3`。
- MCU 与板载外设为 `3.3 V` 逻辑。外接模块代码和接线默认按 3.3 V IO 设计；不要在未确认 5 V 容忍能力时直接接 5 V 信号。
